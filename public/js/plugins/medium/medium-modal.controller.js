...
module.exports =
  ...
  vm.orgs  = mediumService.config.orgs;
  vm.step  = 1;
  ...
  function setRepos() {
    ...
    vm.step  = 2;
    vm.pagination = mediumService.config.pagination;
    ...
    vm.repos = mediumService.config.repos.sort(function(a, b) {
      if (a.name < b.name) {
        return -1;
  ...
  function fetchFile(url, path) {
    mediumService.config.current.fileName = path.split('/').pop();
    mediumService.config.current.path = path;
    mediumService.fetchFile(url).then(setFile);
  ...
  function setBranches() {
    vm.title = 'Branches';
    vm.step = 3;
    vm.branches = mediumService.config.branches;

    return vm.branches;
  }

  function fetchBranches(name) {
    mediumService.config.current.repo = name;
    mediumService.fetchBranches(name).then(setBranches);
  ...
  function setTreeFiles() {
    vm.title = 'Files';
    vm.step  = 4;
    vm.files = mediumService.config.current.tree;

    return vm.files;
  }

  function fetchTreeFiles(sha, branch) {
    mediumService.config.current.sha    = sha;
    mediumService.config.current.branch = branch;
    mediumService.fetchTreeFiles(sha).then(setTreeFiles);
